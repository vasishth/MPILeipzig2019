...
library(lme4)
m<-lmer(log(rawRT)~so+(1+so|subject) + (1+so|item),gge1crit,control=lmerControl(calc.derivs=FALSE))
...
```{r}
compute_power_type2<-function(nsim=100,b=-0.036,
                        nsubj=40,nitem=16){
  tvals<-c()
for(i in 1:nsim){
...
Tasks
  ...
  - Carry out the above steps, and establish what sample size (number of subjects) you need to detect an effect if the estimate of the effect (the slope of the fixed effect) is significant. All the code is provided above for this, you just need to reuse it, adapting the code as needed.
